...
try: from gmpy2 import mpz; mpzv, inttypes =      2, (int, type(mpz(1)))
 ...
 try: from gmpy import mpz; mpzv, inttypes =      1, (int, type(mpz(1)))
    ...
    while True:
        if n not in sieve:
            if n < q: yield n
                ...
                next_value, step = q + 2*p, 2*p
                while next_value in sieve: next_value += step
                sieve[next_value] = step
                p = next(ps)
                q = p**2
        else:
            step = sieve.pop(n)
...
def pollard_pm1(n, B1=0, B2=0):          # TODO: What are the best default bounds and way to increment them?
    if B1 == 0 & B2 == 0:
        B1 = int (log (n, 2))
        B2 = B1 * 10
...
def multifactor(n, methods=(pollardRho_brent, pollard_pm1, williams_pp1, ecm, mpqs), verbose=False, timeout=float("inf")):
    ...
    try:
        (f, g) = factors.get(timeout=timeout)
    except queue.Empty :
...
def rpn(instr):
    ...
    for token in instr.split():
        if set(token).issubset("1234567890"): stack.append(int(token))
            ...
            a = stack.pop()
            if   token == '!' : res = listprod(range(1, a+1))
